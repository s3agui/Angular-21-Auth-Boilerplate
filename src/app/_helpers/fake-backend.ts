...
export class FakeBackendInterceptor implements HttpInterceptor {
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
            ...
            if (accounts.find(x => x.email === account.email)) {
                ...
                setTimeout(() => {
                    alertService.info(`
                        ...
                        <div><strong>NOTE:</strong> The fake backend displayed this "email" directly instead of sending it to your inbox.</div>
                    `, { autoClose: false });
                }, 1000);
        ...
        function getAccountById() {
            ...
            // accounts can get their own account and admins can get any account
        ...
        function updateAccount() {
            ...
            // accounts can update their own account and admins can update any account
            if (account.id !== currentAccount()?.id && !isAdmin()) return unauthorized();
        ...
        function deleteAccount() {
            ...
            // accounts can delete their own account and admins can delete any account
            if (account.id !== currentAccount()?.id && !isAdmin()) return unauthorized();
        ...
        function error(message: string) {
            return throwError(() => ({ error: { message } }))
                .pipe(materialize(), delay(500), dematerialize()); // delay observable to simulate server api call
        }
        ...
        function getRefreshToken() {
            ...
            return document.cookie.split(';').find(x => x.trim().startsWith('fakeRefreshToken='))?.split('=')[1];
        }
